...
.*Before you begin:*
. Make sure you have completed Exercise 5. 
...
In the previous Exercises, you wrote queries to perform additional filtering, count and collect results, as well as to perform some intermediate processing during the query.
...
. You want to know what actors acted in movies in the decade starting with the year 1990. First write a query to retrieve all actors that acted in movies during the 1990's, where you return the released date, the movie title, and the collected actor names for the movie. For now do not worry about duplication.
...
*1. You want to know what actors acted in movies in the decade starting with the year 1990. First write a query to retrieve all actors that acted in movies during the 1990's, where you return the released date, the movie title, and the collected actor names for the movie. For now do not worry about duplication.*
...
*2. The results returned from the previous query include multiple rows for a movie released value. Next, modify the query so that the released date records returned are not duplicated. To implement this, you must add the collection of the movie titles to the results returned.*
...
*3. The results returned from the previous query returns the collection of movie titles with duplicates. That is because there are multiple actors per released year. Next, modify the query so that there is no duplication of the movies listed for a year.*
...
. Retrieve the top 5 movies based upon their ratings, returning the movie title and the rating.
. Retrieve the actors in the graph and at most 3 movies they acted in as a list.
...
*1. Retrieve the top 5 movies based upon their ratings, returning the movie title and the rating.*
...
*2. Retrieve the actors in the graph and at most 3 movies they acted in as a list.*
