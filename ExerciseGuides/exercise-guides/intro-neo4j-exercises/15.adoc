...
FROM 'http://data.neo4j.com/intro-neo4j/actors.csv'
AS line
RETURN line.id, line.name, toInteger(lTrim(line.birthYear))
----
...
MERGE (actor:Person {name: line.name})
  ON CREATE SET actor.born = toInteger(lTrim(line.birthYear)), actor.actorId = line.id
...
FROM 'http://data.neo4j.com/intro-neo4j/movies.csv'
AS line
RETURN line.id, line.title, toInteger(line.year), lTrim(line.tagLine)
----
...
ON CREATE 
  SET m.released = toInteger(lTrim(line.year)), 
      m.movieId = line.id,
