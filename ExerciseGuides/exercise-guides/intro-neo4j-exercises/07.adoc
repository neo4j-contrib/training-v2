...
.*Before you begin:*
. Make sure you have completed Exercise 6. 
...
In the previous Exercises, you wrote queries using Cypher syntax for retrieving, filtering, ordering, avoiding duplication, and performing intermediate processing during a query. These queries give you a good basis for exploring more types of queries against the _Movie_ database you are working with for this training.
...
*1. Write a Cypher query that retrieves all actors that acted in movies, and collects the list of movies for any actor that acted in more than five movies.  Return the name of the actor and the list.*
...
In this view, we are examining the table view where we can see the actor name and the list of movies for each actor. The actors should be _Tom Hanks_ and _Keanu Reeves_.
