...
.*Before you begin:*
. Make sure you have completed Exercise 7. 
...
. Retrieve the node you just created by its _id_.
...
*Note:* The value for the _id_ property for the newly-created node may be different for your graph. It this screen shot, it is _741_.

++++
<div style="clear: both;"></div>
++++

*3. Retrieve the node you just created by its _id_.*

[source, cypher]
----
MATCH (m:Movie)
// the number you specify here depends on your particular graph
// and is based upon what you see when you retrieve by _title_
WHERE id(m) = 741
RETURN m
----

[.thumb]
image::{guides}/img/RetrieveForrestGumpByID.png[RetrieveForrestGumpByID,width=400,float="right"]
...
*4. Create a _Person_ node for the person with the _name_, _Robin Wright_.*
...
. Retrieve all _Female_ nodes. Do you notice that not all of these nodes should be labeled _Female_?
...
*4. Retrieve all _Female_ nodes. Do you notice that not all of these nodes should be labeled _Female_?*
...
Here we see that there are two _Person_ nodes. The _Robin Williams_ node should not have the label _Female_.

++++
<div style="clear: both;"></div>
++++

*5. We've decided to not use the _Female_ label. Remove the _Female_ label from the nodes that have this label.*
...
In this Exercise, you wrote Cypher statemewnts to create nodes. Then you wrote Cypher statements to add and remove labels for nodes. Next, you used Cypher to add properties to nodes. Finally, you wrote Cypher statements to remove properties from nodes.
