...
.. You have created a graph in a project you will use for training purposes.
.. You have started the database.
.. For the Neo4j Browser application, ensure that _Enable multi-statement query editor_ is selected in the browser settings. If you are using the Web interface for Neo4j Browser, this setting is assumed.
...
.. If the graph does not have the same number of nodes and relationships, you can run the script below that resets the database. Run the script by clicking the script. It will then appear in your query editor pane where you simply run it by clicking the _Play_ button.
...
* If no nodes are returned, then perhaps you did not load the _Movie_ database. (See _Before you begin_).
...
*1. In the query editor pane, enter the Cypher query to return the schema of the database. Do you see the labels in the schema of the graph?*
...
Notice also, that relationships are displayed. 
