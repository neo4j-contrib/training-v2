...
*In the monitoring Neo4j Session, execute this code that attempts to lock the same node:*
...
*You can terminate this node in the monitoring Neo4j Browser session by simply click the "X" in the top right corner of the window.*
...
When you see that this simple query hangs, click the "X" to kill it in the monitoring Neo4j Browser session:
...
*In the monitoring Neo4j Browser session, write code to monitor locking.*

*In the main Neo4j Browser window, recall the last statement and re-execute it to hold the write lock for 200 seconds.*

*In the monitoring Neo3j Browser session, execute the code to monitor locking.*
