...
*Regardless of what is in  your graph, run the following script which removes all nodes and relationships from the graph and loads the graph with the "movie" data by executing this Cypher code:*
...
*Execute this cypher code and then answer these questions*:
...
*What is the anchor of this query?*

*How many paths are traversed during this query?*
...
*What are the anchors for each MATCH clause?*
...
One path is traversed for the first MATCH clause. Six paths are traversed for the second MATCH clause. Notice that the path from Rita Wilson to the movie Sleepless in Seattle is traversed twice.
...
*Modify this query to use a single anchor and thus reduce the number of paths traversed.*
...
*We want a query that finds all people who have acted in at least 5 movies. If this found person has also directed a movie, display the person's name and the title of the movie. This code is not correct. Correct it so that it returns the correct results.*
...
Here is the solution. We also want to carry the p variable for the optional match. These actors acted in at least 5 movies, but only Tom Hanks also directed a movie.
...
*Here is query to retrieve the movies and co-actors of Rita Wilson. This query does not return the Rita Wilson node as a coactor. This is because Neo4j does not traverse a relationship path more than once. Modify this query so that the Rita Wilson node will also be returned*
...
*Write a query to return the shortest path between the actors Meg Ryan and Al Pacino.*

*PROFILE the query to see how the query executes.*
