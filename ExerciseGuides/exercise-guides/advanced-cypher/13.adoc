...
*Here is a query that retrieves the name for the actor, Meg Ryan and the movies she has acted in that contain the word Love. Execute this query and note whether an index is used.*
...
*Rewrite this query to improve its performance.*
...
[.thumb]
image::{guides}/img/EX13.1.png[EX13.1,width=300]
...
[.thumb]
image::{guides}/img/EX13.1C.png[EX13.1C,width=300]
...
*Execute this Cypher code to retrieve the titles of movies that Tom Cruise acted in and Steven Spielberg directed.*
...
*Modify this query to perform better by using two indexes.*
...
[.thumb]
image::{guides}/img/EX13.2.png[EX13.2,width=300]
...
*Write and execute a query using the existing index on the Movie.title property to retrieve all movie titles that contain these strings:
'Part Two'
'Part II'
'Part 2'
*

*Create a fulltext schema index on the Movie title property.*

*Perform the same query using the fulltext schema index.*
...
You cannot use a fulltext schema index on an array type so in order to create an index on a relationship for searching, you will first add a property to all :ACTED_IN relationship that creates a new property, roleList.

*Execute this Cypher code to create this new property for all :ACTED_IN relationships:*
...
*Create a fulltext schema index on the roleList property of the :ACTED_IN relationships.*

*Write and execute the query to find all :ACTED_IN relationships in the database containing the role "Batman" using fulltext schema index search and return the names of the actors and the movies for these roles.*
