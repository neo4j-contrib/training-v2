...
quizesCookie = window.localStorage.getItem('com.neo4j.graphacademy.datascience.' + STAGE + '.quizes');
if (quizesCookie) {
...
function updateQuizStatus() {
  ...
  if (passedArray.length == 3) {
    $('#module-5').find('#quizes-result').html("<p>All quizes taken successfully.</p>");
  } else {
    $('#module-5').find('#quizes-result').html("<p>Some quizes not answered successfully.  Return to course modules by clicking on the numbers  in the navigation at the top of the page.</p>");
  }
...
function getQuizStatus() {
  return getQuizStatusRemote().then( function( value ) { 
    ...
    window.localStorage.setItem('com.neo4j.graphacademy.datascience.' + STAGE + '.quizes', JSON.stringify(quizesStatusL) );
