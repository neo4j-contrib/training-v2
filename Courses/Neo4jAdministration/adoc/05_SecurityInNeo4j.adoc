...
For training purposes, we will use an LDAP provider for authentication, but in your real application, you will need to configure Neo4j for whatever provider you use for user authentication.
...
Using AD with SAMAccountName gives you the greatest flexibility because with this configuration, you do not need to specify a fixed _dn_ template for authentication. Most enterprise deployments use this option.
...
| The system account will log in and search for the user based upon their _samaccountname_ and will do both group membership as well as return the users _dn__ so that bind (authentication) can be completed for the user.
|====
{set:cellbgcolor!}
