...
. In a terminal window, modify the *neo4j.conf* file for the stand-alone Neo4j instance to use the *movie3.db*, rather than the *crimes.db*.

*Exercise steps*:

. Modify the *neo4j.conf* file to create a log record if a query exceeds 1000 ms.

image::{imagedir}/L06-Ex1-LogQueriesTooLong.png[L06-Ex1-LogQueriesTooLong,width=800,align=center]
...
image::{imagedir}/L06-Ex1-QueryLog.png[L06-Ex1-QueryLog,width=800,align=center]
...
image::{imagedir}/L06-Ex1-ListTransactions.png[L06-Ex1-ListTransactions,width=800,align=center]

[start=11]
. In the second _admin_ `cypher-shell` session, execute the Cypher statement to kill the long-running query.

image::{imagedir}/L06-Ex1-KillQuery.png[L06-Ex1-KillQuery,width=800,align=center]
...
image::{imagedir}/L06-Ex2-LongTransactionTimeOutLog.png[L06-Ex2-LongTransactionTimeOutLog,width=800,align=center]
...
. Make sure the *write.sh* has execute permissions (`chmod +x write.sh`)
...
image::{imagedir}/L06-Ex3-ListConnections1.png[L06-Ex3-ListConnections1,width=800,align=center]
...
image::{imagedir}/L06-Ex3-ListConnections2.png[L06-Ex3-ListConnections2,width=800,align=center]
...
image::{imagedir}/L06-Ex3-ListConnections3.png[L06-Ex3-ListConnections3,width=800,align=center]
...
image::{imagedir}/L06-Ex3-ListConnections4.png[L06-Ex3-ListConnections4,width=800,align=center]

[start=9]
. In the _admin_ `cypher-shell` session, enter the Cypher statement to kill the java client connections for _publisher_.

image::{imagedir}/L06-Ex3-KillConnection.png[L06-Ex3-KillConnection,width=800,align=center]
...
image::{imagedir}/jcmd.png[jcmd,width=800,align=center]
...
[start=9]
. If we want to add 1.3 million nodes to this database, we need to adjust the memory requirements to be at a minimum what we see from `memrec`.  In *neo4j.conf*, modify  `dbms.memory.heap.initial_size`, `dbms.memory.heap.max_size` , and `dbms.memory.pagecache.size` values to reflect what you see from `memrec`.
...
Metrics are collected in CSV format by default on disk, but Neo4j supports Graphite and Prometheus as protocols to publish those metrics out to remote services.  A typical way that customers set this up is to configure Prometheus and then have some external service pull the metrics as needed from Neo4j.  In this way, you don't need to set anything up around CSV metrics, but you can readily integrate Neo4j with any other monitoring application that can utilize Prometheus or Graphite. Examples of those include Stackdriver, Grafana, and DataDog. You typically set up these remote services for 24/7 monitoring and alerting.
...
https://halin.graphapp.io/[Halin] has been developed for querying the Neo4j instance. Here are a couple of screen shots when using Halin for viewing metrics:
...
- [ ] [.required-answer]#Publish to an endpoint using the Graphite protocol.#
- [ ] [.required-answer]#Publish to an endpoint using the Prometheus protocol.#
