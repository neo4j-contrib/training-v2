...
As a database administrator, you may need to automate changes to the database. You can create scripts that send the Cypher statements to `cypher-shell`.  To do this, you create 2 files:

. *ScriptName.sh* that will invoke `cypher-shell`.
. *CypherStatements.cypher* that will contain the Cypher statements to execute in `cypher-shell`.

In this example, we have created a script, *DropConstraints.sh*. This script forwards the Cypher statements in *DropConstraints.cypher* to `cypher-shell`.  In the *.cypher* file, you must ensure that a semi-colon ends every Cypher statement. 

Here is the *PrepareDB.sh* file:

image::{imagedir}/PrepareDB.sh.png[PrepareDB.sh,width=800,align=center]

Here is the *DropConstraints.cypher* file:

image::{imagedir}/DropConstraints.cypher.png[DropConstraints.cypher,width=800,align=center]

After the script runs, we verify it  by accessing a cypher-script session that indeed the constraints no longer exist in the database.

image::{imagedir}/Cypher-shell2.png[Cypher-shell2,width=800,align=center]
