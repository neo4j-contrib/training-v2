...
Then, if you want to create a database from any offline backjp file to use for a Neo4j instance, you must:
...
[NOTE]
Dumping and loading a database is done when the Neo4j instance is stopped. Later in this lesson, you will learn about online backup and restore. Offline backup is typically done for initial setup and development purposes. Online backup and restore is done in a production environment.
...
image::{imagedir}/L03-Ex3-movie-cfg.png[x3-movie-cfg,width=1000,align=center]
...
image::{imagedir}/L03-Ex4-LocationConfig.png[Ex4-LocationConfig,width=800,align=center]
...
image::{imagedir}/L03-Ex4-ActiveDatabase.png[Ex4-ActiveDatabase,width=800,align=center]
...
image::{imagedir}/L03-Ex4-LoadDB.png[Ex4-LoadDB,width=800,align=center]
...
image::{imagedir}/L03-Ex4-Access.png[Ex4-Access,width=1000,align=center]
...
image::{imagedir}/L03-Ex4-Access2.png[Ex4-Access2,width=1000,align=center]
...
The database named _db-name_ is found in the data location specified in *neo4j.conf* file. If the tool comes back with no error, then the database is consistent. Otherwise, an error is returned and a report is written to _report-location_. You can specify verbose reporting. See the _Neo4j Operations Manual_ for more options. For example, you can check the consistency of a backup. 
...
image::{imagedir}/L03-Ex6-StartCypher-shell.png[Ex6-StartCypher-shell,width=800,align=center]
...
image::{imagedir}/L03-Ex6-RunMaintainDB.png[Ex6-RunMaintainDB,width=800,align=center]

[start=10]
. Confirm that it created the constraints in the database. (Check using cypher-shell (`CALL db.constraints();`))

image::{imagedir}/L03-Ex6-ConfirmConstraints.png[Ex6-ConfirmConstraints,width=800,align=center]
...
image::{imagedir}/L03-Ex7-SpatialQuery1.png[Ex7-SpatialQuery1,width=800,align=center]

Here is what you should see when you execute the second `curl` command:

image::{imagedir}/L03-Ex7-SpatialQuery2.png[Ex7-SpatialQuery2,width=800,align=center]
...
image::{imagedir}/L03-Ex7-SpatialLibraryLoaded.png[Ex7-SpatialLibraryLoaded,width=800,align=center]
...
[start=2]
. Restart the Neo4j instance.
. Confirm that the port works by entering the following `curl` command that uses the Neo4h HTTP API to create a node, where it will ask you for the password the _neo4j_ user:
...
image::{imagedir}/L03-Ex8-HTTP9999-used.png[Ex8-HTTP9999-used,width=1000,align=center]
...
----
neo4j-admin restore  
                   --from=<absolute-path-to-backup-files>
...
image::{imagedir}/L03-Ex9-Backup-config.png[Ex9-Backup-config,width=800,align=center]
...
image::{imagedir}/L03-Ex9-Backup1.png[Ex9-Backup1,width=800,align=center]

image::{imagedir}/L03-Ex9-Backup2.png[Ex9-Backup2,width=800,align=center]
...
image::{imagedir}/L03-Ex9-Inconsistency.png[Ex9-Inconsistency,width=800,align=center]
...
image::{imagedir}/L03-Ex9-Restore.png[Ex9-Restore,width=800,align=center]
...
. Navigate to the *import* folder and download this file: https://s3-us-west-1.amazonaws.com/data.neo4j.com/admin-neo4j/crime-data.zip
...
image::{imagedir}/L03-Ex10-Import1.png[Ex10-Import1,width=800,align=center]

image::{imagedir}/L03-Ex10-Import2.png[Ex10-Import2,width=800,align=center]

[start=7]
. Modify the *neo4j.conf* file to use *crimes.db* as the active database.
. Ensure that the ownership of the crimes.db directory and everything under it is owned by neo4j:neo4j.
. Start the Neo4j instance.
. Run `cypher-shell` to retrieve the schema of the database and also count the number of _Crime_ nodes in the graph.

image::{imagedir}/L03-Ex10-QueryCrimesDB.png[Ex10-QueryCrimesDB,width=800,align=center]
