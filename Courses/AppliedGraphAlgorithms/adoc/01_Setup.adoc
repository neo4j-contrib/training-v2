...
This plugin is available in the Yelp Sandbox by default so you won't need to worry about installing.
...
This CodeSandbox contains all the client code for the application that you will build. You will want to have CodeSandbox open in another tab as you work through the application.
...
. In the *Get Started* tab of the Yelp Sandbox, click the *Launch Browser* button to open a Neo4j Browser for the Yelp database.
. Complete the first Neo4j Browser Guide (:play applied_graph_algorithms) "Cypher Refresher".
...
.. Replace the value for REACT_APP_NEO4J_URI with the value of *JavaScript Bolt URL* from the *Details* tab of the Yelp Sandbox.
.. Replace the value for REACT_APP_NEO4J_PASSWORD with the *Password* in the *Details* tab of the Yelp Sandbox.
. Save the *.env* file.
    *Note*: After you have saved your changes to the *.env* file , CodeSandbox automatically forks this instance for you.
. Click the refresh icon to connect the CodeSandbox Browser (on the right) to the database.
. Confirm that you can now view data in the database by selecting a name in the drop down (initially selected with Delores). Data should be retrieved for each user.
