...
. Save this file. Then perform a search on behalf of as user by first selecting a user. Then select the restaurant category and in the search pane, specify the string *Mamma* for the name of the restaurant. This will search for restaurants that start with that name that have reviews that are  most relevant to the user with the remaining reviews in chronological order. You can select a restaurant to see the reviews.
